...
LOOM_TOKEN=approver-admin-token go run ./cmd/loom exec approval.issue \
  --boundary=dev \
  --idempotency-key="iss-demo-$(date +%s)" \
  --input='{"principal":"user:bob","operation":"payment.capture","boundary":"dev","token":"demo-appr-1","ttl_seconds":3600}'

echo "==> payment.capture (bob + approval)"
LOOM_TOKEN=bob-finance-token go run ./cmd/loom exec payment.capture \
  --boundary=dev \
  --resource-type=payment --resource-id=demo-pay \
  --idempotency-key="pay-demo-$(date +%s)" \
  --approval=demo-appr-1 \
  --input='{"amount":25.5,"currency":"USD","merchant_id":"m-demo"}'

echo "==> alice cannot payment.capture (expect deny)"
LOOM_TOKEN=alice-secret-token go run ./cmd/loom exec payment.capture \
  --boundary=dev \
